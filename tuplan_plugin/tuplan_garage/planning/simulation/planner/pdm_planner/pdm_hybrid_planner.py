...
class PDMHybridPlanner(AbstractPDMClosedPlanner):
    ...
    def compute_planner_trajectory(
        ...
        # # PDM Projection Mode
        # # if you want to use this mode, please comment out the below PDM Offset Mode
        # modifie_states = []
        # for corrected_state in corrected_states:
        #     corrected_array = corrected_state.center.array
        #     min_dis = 10000
        #     for uncorrected_state in uncorrected_states:
        #         uncorrected_array = uncorrected_state.center.array
        #         dis = np.linalg.norm(corrected_array-uncorrected_array)
        #         if dis <= min_dis:
        #             modifie_state = EgoState(
        #                 car_footprint = uncorrected_state.car_footprint,
        #                 dynamic_car_state = uncorrected_state.dynamic_car_state,
        #                 tire_steering_angle = uncorrected_state.tire_steering_angle,
        #                 is_in_auto_mode = uncorrected_state.is_in_auto_mode,
        #                 time_point = corrected_state.time_point,
        #             )
        #             min_dis = dis
        #     modifie_states.append(modifie_state)
        # trajectory = self._apply_trajectory_correction(
        #     uncorrected_states, corrected_states
        # )

        # PDM Offset Mode
        # if you want to use this mode, please comment out the above PDM Projection Mode
        trajectory = InterpolatedTrajectory(corrected_states)
